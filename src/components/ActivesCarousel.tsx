      ...
      <div className="container-x">
        ...
        <div className="mt-6 rounded-2xl bg-ink px-6 py-6 text-primary-foreground">
          <div className="grid gap-5 sm:grid-cols-3 lg:grid-cols-[repeat(5,1fr)_1.4fr] lg:items-center">
            ...
            <div className="border-t border-primary-foreground/20 pt-4 text-center text-xs lg:border-l lg:border-t-0 lg:pt-0 lg:pl-6">
        ...
        <div className="mt-7 grid grid-cols-2 gap-5 md:grid-cols-4">
          {actives.map((active) => (
            <article key={active.name}>
              <img src={active.image} alt={active.name} className="aspect-[4/3] w-full rounded-lg object-cover" />
              <h3 className="mt-3 font-display text-xl text-ink">{active.name}</h3>
              <p className="mt-2 text-sm text-foreground">{active.description}</p>
            </article>
          ))}
        </div>
      </div>
